...
class PatternDetector(BaseDetector):
    ...
    def _compile_patterns(self) -> None:
        raw = [
            ...
            # Indian Identifiers
            (r"\b\d{4}\s\d{4}\s\d{4}\b", SensitivityCategory.PERSONAL_INFO, 0.95), # Aadhaar
            (r"\b\d{12}\b", SensitivityCategory.PERSONAL_INFO, 0.80), # 12-digit potential Aadhaar
            (r"\b[A-Z]{5}[0-9]{4}[A-Z]{1}\b", SensitivityCategory.FINANCIAL_DATA, 0.95), # PAN
            (r"\b\d{2}[A-Z]{5}\d{4}[A-Z]{1}\d[Z][A-Z\d]\b", SensitivityCategory.SECRET_BUSINESS, 0.95), # GSTIN
            (r"\b[A-Z]{4}0[A-Z0-9]{6}\b", SensitivityCategory.FINANCIAL_DATA, 0.90), # IFSC
            # Financial Extras
            (r"\b(?:cvv|cvc)\s*[:#]?\s*\d{3,4}\b", SensitivityCategory.FINANCIAL_DATA, 0.90),
            # Business Secrets
            (r"\bacquisition\s+agreement\b", SensitivityCategory.SECRET_BUSINESS, 0.85),
            (r"\bmerger\s+agreement\b", SensitivityCategory.SECRET_BUSINESS, 0.85),
        ]
