...
class GLiNERDetector(BaseDetector):
    ...
    def detect(self, text: str, context: Optional[dict] = None) -> DetectionResult:
        ...
        try:
            custom = (context or {}).get("custom_labels")
            base_labels = self._get_labels()
            labels = (base_labels + list(custom)) if custom else base_labels
            labels = [x for x in labels if (x and isinstance(x, str) and x.strip())]
        ...
        for e in raw:
            ...
            # Robustness check: Ignore short strings for credentials to prevent false positives like "he he"
            if cat == SensitivityCategory.CREDENTIALS and len(value) < 10:
                continue
            entities.append(
                Entity(
