...
from securegate.document_upload import extract_text_from_bytes
...
def _build_detectors() -> list:
    ...
    if "custom_llm" in enabled:
        try:
            detectors.append(CustomLLMDetector())
        except Exception as e:
            logger.warning("Custom LLM detector skipped: %s", e)
...
async def analyze_text(body: dict) -> dict:
    """Convenience endpoint: analyze plain text. Body: {"text": "...", "custom_labels": ["label1", ...]}."""
    text = body.get("text", "")
    custom_labels = body.get("custom_labels")
    if custom_labels is not None and not isinstance(custom_labels, list):
        custom_labels = [str(c).strip() for c in str(custom_labels).split(",")] if custom_labels else None
    req = AnalysisRequest(text=text, custom_labels=custom_labels)
...
async def upload_document(file: UploadFile = File(...)) -> dict:
    """Extract text from PDF or text file and return suggested labels (e.g. unique lines)."""
    content = await file.read()
    if not content:
        raise HTTPException(status_code=400, detail="Empty file")
    filename = file.filename or "upload"
    try:
        text, suggested_labels = extract_text_from_bytes(content, filename)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Extraction failed: {e!s}")
    return {"text": text, "suggested_labels": suggested_labels, "filename": filename}
...
<body class="bg-slate-900 text-slate-100 min-h-screen p-6">
  ...
  <script>
    ...
    async function load() {
      ...
      const actionColors = { Block: '#ef4444', Mask: '#f59e0b', Allow: '#22c55e' };
      const actionOrder = ['Block','Mask','Allow'];
      ...
      const html = events.length ? events.map((e, i) => {
        const act = e.action || '-';
        const cls = act === 'Block' ? 'text-red-400' : act === 'Mask' ? 'text-amber-400' : 'text-emerald-400';
